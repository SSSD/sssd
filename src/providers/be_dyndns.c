...
get_sss_iface_addr(struct sss_iface_addr *list, struct rtnl_addr *nl_addr)
{
    ...
    if (rtnl_addr_get_family(nl_addr) != AF_INET6) {
        /* we are interested in IPv6 only */
        return NULL;
    }
    ...
    for (addr = list; addr != NULL; addr = sss_iface_addr_get_next(addr)) {
        if (addr->addr->sa_family != AF_INET6) {
            continue;
        }

        sa_bin_addr = &(((struct sockaddr_in6 *)addr->addr)->sin6_addr);
        if (memcmp(nl_bin_addr, sa_bin_addr, sizeof(struct in6_addr)) == 0) {
            return addr;
        }
    }
...
static
int sss_iface_addr_list_set_ifa_flags(struct sss_iface_addr *addrlist)
{
