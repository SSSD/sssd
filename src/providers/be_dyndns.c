                                   ...
                                   struct network_pattern *pattern)
{
    ...
    /* negative */
    network_str = network;
    pattern->negative = (network_str[0] == '!');
    while (network_str[0] == '!' || isspace(network_str[0])) {
        ++network_str;
    }
    ...
    /* address */
    if (inet_pton(pattern->family, buffer, &(pattern->address)) != 1) {
        return EINVAL;
    }
                                   ...
                                   struct network_pattern *network)
{
    int bytes, bits, i;
    uint8_t mask;
    const uint8_t *ip_bytes, *net_bytes;
    ...
    case AF_INET:
        ip_bytes = (const uint8_t *)&(ipv4sock->sin_addr.s_addr);
        break;
    case AF_INET6:
        ip_bytes = (const uint8_t *)&(ipv6sock->sin6_addr);
        break;
    default:
        return false;
    }
    ...
    net_bytes = (const uint8_t *)&(network->address);
    ...
    if (bits) {
        mask = 0xFF << (8 - bits);
        if ((ip_bytes[bytes] & mask) != (net_bytes[bytes] & mask)) {
            return false;
        }
    }
