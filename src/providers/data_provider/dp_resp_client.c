...
static void dp_sbus_domain_active_done(struct tevent_req *subreq);
                           ...
                           struct sss_domain_info *dom)
{
    ...
    for (i = 0; all_clients[i] != NULL; i++) {
        ...
        tevent_req_set_callback(subreq, dp_sbus_domain_active_done,
                                provider);
    }
}
...
static void dp_sbus_domain_active_done(struct tevent_req *subreq)
{
    ...
    ret = sbus_call_resp_domain_SetActive_recv(subreq);
...
static void dp_sbus_domain_inconsistent_done(struct tevent_req *subreq);
                                 ...
                                 struct sss_domain_info *dom)
{
    ...
    for (i = 0; all_clients[i] != NULL; i++) {
        ...
        tevent_req_set_callback(subreq, dp_sbus_domain_inconsistent_done,
                                provider);
    }
}
...
static void dp_sbus_domain_inconsistent_done(struct tevent_req *subreq)
{
    ...
    ret = sbus_call_resp_domain_SetInconsistent_recv(subreq);
    if (ret != EOK && ret != ERR_SBUS_UNKNOWN_SERVICE) {
        DEBUG(SSSDBG_OP_FAILURE, "Error sending sbus message [%d]: %s\n",
              ret, sss_strerror(ret));
    }
...
static void dp_sbus_reset_users_ncache_done(struct tevent_req *subreq);
                                ...
                                struct sss_domain_info *dom)
{
    ...
    for (i = 0; user_clients[i] != NULL; i++) {
        ...
        tevent_req_set_callback(subreq, dp_sbus_reset_users_ncache_done,
                                provider);
    }
}
...
static void dp_sbus_reset_users_ncache_done(struct tevent_req *subreq)
{
    ...
    ret = sbus_call_resp_negcache_ResetUsers_recv(subreq);
    if (ret != EOK && ret != ERR_SBUS_UNKNOWN_SERVICE) {
        DEBUG(SSSDBG_OP_FAILURE, "Error sending sbus message [%d]: %s\n",
              ret, sss_strerror(ret));
    }
...
static void dp_sbus_reset_groups_ncache_done(struct tevent_req *subreq);
                                 ...
                                 struct sss_domain_info *dom)
{
    ...
    for (i = 0; user_clients[i] != NULL; i++) {
        ...
        tevent_req_set_callback(subreq, dp_sbus_reset_groups_ncache_done,
                                provider);
    }
}
...
static void dp_sbus_reset_groups_ncache_done(struct tevent_req *subreq)
{
    ...
    ret = sbus_call_resp_negcache_ResetGroups_recv(subreq);
    if (ret != EOK && ret != ERR_SBUS_UNKNOWN_SERVICE) {
        DEBUG(SSSDBG_OP_FAILURE, "Error sending sbus message [%d]: %s\n",
              ret, sss_strerror(ret));
    }
...
static void dp_sbus_reset_users_memcache_done(struct tevent_req *subreq);
...
void dp_sbus_reset_users_memcache(struct data_provider *provider)
{
    ...
    tevent_req_set_callback(subreq, dp_sbus_reset_users_memcache_done,
                            provider);
...
static void dp_sbus_reset_users_memcache_done(struct tevent_req *subreq)
{
    ...
    ret = sbus_call_nss_memcache_InvalidateAllUsers_recv(subreq);
    if (ret != EOK && ret != ERR_SBUS_UNKNOWN_SERVICE) {
        DEBUG(SSSDBG_OP_FAILURE, "Error sending sbus message [%d]: %s\n",
              ret, sss_strerror(ret));
    }
...
static void dp_sbus_reset_groups_memcache_done(struct tevent_req *subreq);
...
void dp_sbus_reset_groups_memcache(struct data_provider *provider)
{
    ...
    tevent_req_set_callback(subreq, dp_sbus_reset_groups_memcache_done,
                            provider);
...
static void dp_sbus_reset_groups_memcache_done(struct tevent_req *subreq)
{
    ...
    ret = sbus_call_nss_memcache_InvalidateAllGroups_recv(subreq);
    if (ret != EOK && ret != ERR_SBUS_UNKNOWN_SERVICE) {
        DEBUG(SSSDBG_OP_FAILURE, "Error sending sbus message [%d]: %s\n",
              ret, sss_strerror(ret));
    }
...
static void dp_sbus_reset_initgr_memcache_done(struct tevent_req *subreq);
...
void dp_sbus_reset_initgr_memcache(struct data_provider *provider)
{
    ...
    tevent_req_set_callback(subreq, dp_sbus_reset_initgr_memcache_done,
                            provider);
...
static void dp_sbus_reset_initgr_memcache_done(struct tevent_req *subreq)
{
    ...
    ret = sbus_call_nss_memcache_InvalidateAllInitgroups_recv(subreq);
    if (ret != EOK && ret != ERR_SBUS_UNKNOWN_SERVICE) {
        DEBUG(SSSDBG_OP_FAILURE, "Error sending sbus message [%d]: %s\n",
              ret, sss_strerror(ret));
    }
...
static void dp_sbus_invalidate_group_memcache_done(struct tevent_req *subreq);
                                       ...
                                       gid_t gid)
{
    ...
    tevent_req_set_callback(subreq, dp_sbus_invalidate_group_memcache_done,
                            provider);
...
static void dp_sbus_invalidate_group_memcache_done(struct tevent_req *subreq)
{
    ...
    ret = sbus_call_nss_memcache_InvalidateGroupById_recv(subreq);
    if (ret != EOK && ret != ERR_SBUS_UNKNOWN_SERVICE) {
        DEBUG(SSSDBG_OP_FAILURE, "Error sending sbus message [%d]: %s\n",
              ret, sss_strerror(ret));
    }
