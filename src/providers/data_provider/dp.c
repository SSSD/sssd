...
dp_init_interface(struct data_provider *provider)
{
    ...
    struct sbus_interface iface_service = SBUS_INTERFACE(
        sssd_service,
        SBUS_METHODS(
            SBUS_SYNC(METHOD, sssd_service, resInit, data_provider_res_init, provider->be_ctx),
            SBUS_SYNC(METHOD, sssd_service, goOffline, data_provider_go_offline, provider->be_ctx),
            SBUS_SYNC(METHOD, sssd_service, resetOffline, data_provider_reset_offline, provider->be_ctx),
            SBUS_SYNC(METHOD, sssd_service, rotateLogs, data_provider_logrotate, provider->be_ctx)
        ),
        SBUS_SIGNALS(SBUS_NO_SIGNALS),
        SBUS_PROPERTIES(SBUS_NO_PROPERTIES)
    );
