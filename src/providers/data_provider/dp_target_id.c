...
static void dp_req_initgr_pp_nss_notify_done(struct tevent_req *subreq);
static void dp_req_initgr_pp_nss_notify(struct data_provider *provider,
                                        struct dp_initgr_ctx *ctx)
{
    ...
    subreq = sbus_call_nss_memcache_UpdateInitgroups_send(provider,
                 provider->sbus_conn, SSS_BUS_NSS, SSS_BUS_PATH,
                 ctx->username, ctx->domain, ctx->groups);
    if (subreq == NULL) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Unable to create subrequest!\n");
        return;
    }

    tevent_req_set_callback(subreq, dp_req_initgr_pp_nss_notify_done,
                            provider);
...
static void dp_get_account_info_done(struct tevent_req *subreq)
{
    ...
    dp_req_initgr_pp_nss_notify(state->provider, state->initgr_ctx);
