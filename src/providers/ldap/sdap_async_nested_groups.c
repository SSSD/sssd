                                   ...
                                   struct sdap_nested_group_member *member)
{
    ...
    /* Collect attributes from user_map, skip objectclass */
    ...
    /* Collect attributes from group_map, skip objectclasses */
    ...
    /* Collect attributes from fsp_map if it exists, skip objectclass */
    ...
    maps = talloc_array(state, struct sdap_attr_map_info_ex, num_maps +1);
    if (maps == NULL) {
        DEBUG(SSSDBG_OP_FAILURE,
    ...
    if (sdap_idmap_domain_has_algorithmic_mapping(group_ctx->opts->idmap_ctx,
                                                  group_ctx->domain->name,
                                                  group_ctx->domain->domain_id)) {
        maps[0].required_attrs[1] = NULL;
    } else {
        maps[0].required_attrs[1] = group_ctx->opts->user_map[SDAP_AT_USER_UID].name;
        maps[0].required_attrs[2] = group_ctx->opts->user_map[SDAP_AT_USER_GID].name;
        maps[0].required_attrs[3] = NULL;
    }

    maps[1].map = group_ctx->opts->group_map;
    maps[1].num_attrs = SDAP_OPTS_GROUP;
    maps[1].map_type = SDAP_NESTED_GROUP_DN_GROUP;
    maps[1].required_attrs[0] = group_ctx->opts->group_map[SDAP_AT_GROUP_NAME].name;
    maps[1].required_attrs[1] = NULL;
