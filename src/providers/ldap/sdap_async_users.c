                   ...
                   bool set_non_posix)
{
	...
	/* create canonical UPN from sAMAccountName to help Samba and also to allow us to lookup user via UPN */
	...
	if (ret == EOK) {
	    ...
	    if (realm != NULL) {
               new_upn = talloc_asprintf(memctx, "%s@%s", samaccountname, realm);
	       if (new_upn != NULL){
