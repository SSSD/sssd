            ...
            struct ipa_id_ctx **_ipa_id_ctx)
{
   ...
   ret = sdap_domain_subdom_add(ipa_subdom_id_ctx->sdap_id_ctx,
    ...
    ret = domain_to_basedn(be_ctx,
                           dp_opt_get_string(ipa_options->id->basic, SDAP_KRB5_REALM),
                           &basedn);
    if (ret != EOK) {
...
static errno_t ipa_check_keytab(const char *keytab)
{
    ...
    ret = check_file(keytab, -1, -1, S_IFREG|0600, 0, NULL, false);
    if (ret != EOK) {
        DEBUG(SSSDBG_TRACE_FUNC, "check_file(%s) failed: [%d]\n", keytab, ret);
    }
