/*
    ...
    AD Identity Backend Module for sub-domains - evaluate external group
    memberships
...
IMPORTANT: content has been mostly copied from the IPA provider, it does not work.
           the idea is/was to use IFP/dbus provider (something like "FindBySID()")
	   to try other defined responders to resolve the given SID.
	   Obviously it would be better to discuss this with AD GC, but we are
	   not there yet....
...
static void ad_ext_group_member_done(struct tevent_req *subreq);
                                            ...
                                            void *pvt)
{
    ...
    ad_ctx = talloc_get_type(pvt, struct ad_id_ctx);
    if (ad_ctx == NULL) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Wrong private context!\n");
        ret = EINVAL;
        goto immediate;
    }
/*    ret = confdb_get_domains(ad_ctx->sdap_id_ctx->be->cdb, &dom); */
    dom = get_domains_head(ad_ctx->sdap_id_ctx->be->domain);
/*    if (ret != EOK) {
    ...
    while (dom) {
	if (dom != ad_ctx->sdap_id_ctx->be->domain) {
	   DEBUG(SSSDBG_IMPORTANT_INFO,
                      "Trying Domain '%s', provider '%s'\n", dom->name,dom->provider);
           state->dom = dom;
	   break; 
	   state->dom = find_domain_by_sid(dom, ext_member);
           if (state->dom != NULL)
              break;
	}   
...
*/
    ...
    DEBUG(SSSDBG_CRIT_FAILURE, "sending request1!\n");
    subreq = dp_req_send(state, ad_ctx->sdap_id_ctx->be->provider,
                         ar->domain, "External Member", 0, NULL,
                         DPT_ID, DPM_ACCOUNT_HANDLER, 0, ar, NULL);
    DEBUG(SSSDBG_CRIT_FAILURE, "sending request2!\n");
    if (subreq == NULL) {
        ret = ENOMEM;
        goto immediate;
    }
    tevent_req_set_callback(subreq, ad_ext_group_member_done, req);
...
static void ad_ext_group_member_done(struct tevent_req *subreq)
{
    ...
    DEBUG(SSSDBG_CRIT_FAILURE, "reply request1!\n");
