...
class TestNsAccountLock(object):
    ...
    def test_user_inactivated_locked(multihost):
        ...
        with pytest.raises(AssertionError):
            check_login("foo1@example1", multihost.client[0].sys_hostname, "Secret123")
        time.sleep(3)
    ...
    def test_inactive_managed_roles(multihost):
        ...
        assert ret == 'Success'
        with pytest.raises(AssertionError):
            check_login("foo2@example1", multihost.client[0].sys_hostname, "Secret123")
        time.sleep(3)
    ...
    def test_inactivated_filtered_roles(multihost):
        ...
        with pytest.raises(AssertionError):
            check_login("foo3@example1", multihost.client[0].sys_hostname, "Secret123")
        ...
        clean_sys(multihost)
        with pytest.raises(AssertionError):
            check_login("foo4@example1", multihost.client[0].sys_hostname, "Secret123")
