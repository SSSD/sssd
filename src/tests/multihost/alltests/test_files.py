...
class TestFilesProvider(object):
    ...
    def test_003_group_caching(self, multihost, backupsssdconf, local_useradd):
        """
    ...
    def test_004_uid_change(self, multihost, backupsssdconf, local_useradd):
        """
    ...
    def test_005_gid_change(self, multihost, backupsssdconf, local_useradd):
        """
    ...
    def test_006_grp_removal(self, multihost, backupsssdconf, local_useradd):
        """
    ...
    def test_007_zero_gid_user(self, multihost, backupsssdconf, local_useradd):
        ...
        """
        multihost.client[0].service_sssd('start')
        exit_status, _ = run_cmd(multihost, 'local_useradd -g 0 u_zero_uid')
        assert exit_status == 0
        time.sleep(1)
    ...
    def test_010_dup_uid(self, multihost, backupsssdconf):
        ...
        cmd = f'local_useradd -u 2222 {localuser}'
        exit_status, _ = run_cmd(multihost, cmd)
        cmd = f'local_useradd -u 2222 -o {dup_user}'
        exit_status, _ = run_cmd(multihost, cmd)
        time.sleep(1)
    ...
    def test_011_gecos_usr(self, multihost, backupsssdconf):
        ...
        cmd = f'local_useradd -c "{gecos}" {localuser}'
        exit_status, _ = run_cmd(multihost, cmd)
        assert exit_status == 0
        time.sleep(1)
    ...
    def test_012_expired_user(self, multihost, backupsssdconf):
        ...
        multihost.client[0].service_sssd('start')
        lusr = 'usr'
        cmd = f'local_useradd -e 2018-08-09 {lusr}'
        exit_status, _ = run_cmd(multihost, cmd)
        assert exit_status == 0
        time.sleep(1)
