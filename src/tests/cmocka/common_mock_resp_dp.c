...
sss_dp_get_account_recv(TALLOC_CTX *mem_ctx,
                        ...
                        dbus_uint32_t *dp_err)
{
    acct_cb_t cb;

    *dp_err = sss_mock_type(dbus_uint32_t);
...
void mock_resolver_recv(uint32_t dp_err, resolver_cb_t cb,
                        void *pvt)
{
...
{
    return mock_resolver_recv(0, NULL, NULL);
}
...
void mock_account_recv(uint32_t dp_err, acct_cb_t acct_cb,
                       void *pvt)
{
