...
def test_sudo__rules_available_from_cache_when_offline(client: Client, provider: GenericProvider):
    ...
    assert client.auth.sudo.list("user-1", "Secret123", expected=["(ALL) NOPASSWD: ALL"]), "Sudo list failed offline!"
...
def test_sudo__repeated_sigusr2_online_attempts_no_crash(client: Client, provider: GenericProvider):
    ...
    result = client.svc.status("sssd", raise_on_error=False)
...
def test_sudo__full_refresh_downloads_new_rules(client: Client, provider: GenericProvider):
    ...
    :setup:
        ...
        4. Set "ldap_sudo_random_offset" to "0" so the refresh periods are not randomized
           and the test can wait a fixed amount of time for a refresh to happen
        5. Set "cache_credentials" to "True", "entry_cache_timeout" to "0"
        6. Start SSSD
    :steps:
    ...
    assert client.auth.sudo.list(
        "user-1", "Secret123", expected=["(root) NOPASSWD: ALL"]
...
def test_sudo__full_refresh_picks_up_modified_rules(client: Client, provider: GenericProvider):
    ...
    :setup:
        ...
        5. Set "ldap_sudo_random_offset" to "0" so the refresh periods are not randomized
           and the test can wait a fixed amount of time for a refresh to happen
        6. Set "cache_credentials" to "True", "entry_cache_timeout" to "0"
        7. Start SSSD
    :steps:
    ...
    assert client.auth.sudo.list(
        "user-1", "Secret123", expected=["(root) NOPASSWD: /bin/ls"]
    ...
    assert client.auth.sudo.list(
        "user-1", "Secret123", expected=["(root) NOPASSWD: /bin/cat"]
...
def test_sudo__full_refresh_retries_after_startup_offline(client: Client, provider: GenericProvider):
    ...
    :setup:
        ...
        5. Set "ldap_sudo_full_refresh_interval" to "5", "ldap_sudo_smart_refresh_interval" to "1"
        6. Set "ldap_sudo_random_offset" to "0" so the refresh periods are not randomized
           and the test can wait a fixed amount of time for a refresh to happen
        7. Block server before starting SSSD
        8. Start SSSD
    :steps:
