...
@pytest.mark.topology(KnownTopology.Samba)
@pytest.mark.preferred_topology(KnownTopology.LDAP)
def test_netgroup__offline(client: Client, provider: GenericProvider):
    """
    ...
    user = provider.user("user-1").add()
    provider.netgroup("ng-1").add().add_member(
        host="testhost", user=user, domain="ldap.test"
    )
    ...
    assert result.name == "ng-1"
    assert len(result.members) == 1
    assert "(testhost, user-1, ldap.test)" in result.members
...
def test_netgroups__step_by_step_removal(
    ...
    ng_child = provider.netgroup("ng-child").add().add_member(user=u1)
    ng_parent = provider.netgroup("ng-parent").add().add_member(
        user=u2, ng=ng_child
    )
    ...
    ng_parent.remove_member(ng=ng_child)
    client.sssctl.cache_expire(netgroups=True)
...
def test_netgroups__nested_modification(
    ...
    ng_leaf = provider.netgroup("ng-leaf").add().add_member(user=u1)
    ng_middle = provider.netgroup("ng-middle").add().add_member(ng=ng_leaf)
    provider.netgroup("ng-top").add().add_member(user=u2, ng=ng_middle)
