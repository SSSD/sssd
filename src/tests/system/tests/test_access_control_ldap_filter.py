...
thus we are skipping IPA provider
"""
...
@pytest.mark.require(lambda provider: not isinstance(provider, IPA), "Test not valid for this provider")
def test_access_filter__single_ldap_attribute_permits_user_login(client: Client, provider: GenericProvider):
    ...
    if isinstance(provider, (AD, Samba)):
        ...
        client.sssd.domain["ad_access_filter"] = f"(samAccountName={u1.name})"
    else:
        client.sssd.domain["access_provider"] = "ldap"
        client.sssd.domain["ldap_access_filter"] = f"(uid={u1.name})"

    client.sssd.start()

    assert client.auth.ssh.password(u1.name, "Secret123"), f"{u1.name} should be able to login!"
    assert not client.auth.ssh.password(u2.name, "Secret123"), f"{u2.name} should NOT be able to login!"
...
def test_access_filter__ldap_query_with_wildcard_permits_user_login(client: Client, provider: GenericProvider):
    ...
    u1 = provider.user("user1").add(email="user1@allowedLogin.com")
    u2 = provider.user("user2").add(email="user2@deniedLogin.com")
    ...
    assert client.auth.ssh.password(u1.name, "Secret123"), f"{u1.name} should be able to log in!"
    assert not client.auth.ssh.password(u2.name, "Secret123"), f"{u2.name} should NOT be able to log in!"
...
def test_access_filter__ldap_query_with_and_or_not_permits_user_login(client: Client, provider: GenericProvider):
    ...
    u1 = provider.user("user1").add(email="user1@allowedLogin.com")
    u2 = provider.user("user2").add(email="user2@domain.com")
    u3 = provider.user("user3").add(email="user3@deniedLogin.com")
    ...
    assert client.auth.ssh.password(u1.name, "Secret123"), f"{u1.name} should be able to log in!"
    assert client.auth.ssh.password(u2.name, "Secret123"), f"{u2.name} should be able to log in!"
    assert not client.auth.ssh.password(u3.name, "Secret123"), f"{u3.name} should NOT be able to log in!"
...
def test_access_filter__ldap_attributes_approximately_greater_and_less_than_permits_user_login(
    ...
    u1 = provider.user("user1").add(uid=10030)
    u2 = provider.user("user2").add(uid=10025)
