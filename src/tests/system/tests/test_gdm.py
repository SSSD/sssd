...
def test_gdm__xidp_login(client: Client, ipa: IPA, keycloak: Keycloak):
    ...
    :steps:
        ...
        2. Follow IdP redirect url and use valid password to login via External IdP site
        3. Check for home screen to indicate login succeeeded
    :expectedresults:
    """
    testuser = "kcgdmuser1"
    password = "Secret123"
...
def test_gdm__xidp_login_rejected_for_invalid_password(client: Client, ipa: IPA, keycloak: Keycloak):
    ...
    assert not client.gdm.login_idp(client, testuser, password[:-1]), "GDM EIdP Login passed when it should have failed"
...
def test_gdm__xidp_login_get_kerberos_ticket(client: Client, ipa: IPA, keycloak: Keycloak):
    ...
    assert f'krbtgt/{ipa.realm}@{ipa.realm}' in klist.stdout, "Kerberos ticket not found"
