...
def test_sss_override__user(client: Client, provider: GenericProvider):
    """
    :title: Override user
    :setup:
        1. Create user
        2. Configure SSSD with and start SSSD
...
@pytest.mark.topology([KnownTopology.LDAP, KnownTopology.AD, KnownTopology.Samba])
@pytest.mark.preferred_topology(KnownTopology.LDAP)
def test_sss_override__fully_qualified_user_name(client: Client, provider: GenericProvider):
    """
    :title: Override using fully qualified names
    :setup:
        1. Create user
        2. Configure SSSD with "use_fully_qualified_names =True", "ldap_id_mapping=False" and start SSSD
...
def test_sss_override__user_attributes(client: Client, provider: GenericProvider):
    ...
    :setup:
        1. Create user and set posix attributes
...
@pytest.mark.topology([KnownTopology.LDAP, KnownTopology.AD, KnownTopology.Samba])
@pytest.mark.preferred_topology(KnownTopology.LDAP)
def test_sss_override__group(client: Client, provider: GenericProvider):
    """
...
@pytest.mark.topology([KnownTopology.LDAP, KnownTopology.AD, KnownTopology.Samba])
@pytest.mark.preferred_topology(KnownTopology.LDAP)
def test_sss_override__root_account_cannot_be_used(client: Client, provider: GenericProvider):
    """
...
@pytest.mark.topology([KnownTopology.LDAP, KnownTopology.AD, KnownTopology.Samba])
@pytest.mark.preferred_topology(KnownTopology.LDAP)
def test_sss_override__export_import_override_data(client: Client, provider: GenericProvider):
    """
        :title: Export and import override data
    :setup:
        1. Create user and start SSSD
        2. Create local user and group override and restart SSSD
    :steps:
    ...
    :expectedresults:
        ...
        4. User and group local overrides is found
    :customerscenario: False
