...
def test_sss_override__home_takes_precedence_over_override_homedir(client: Client, provider: GenericProvider):
    ...
    :customerscenario: True
    :requirement: IDM-SSSD-TC: ldap_provider: local_overrides: sss_override overrides override_homedir
    """
...
@pytest.mark.topology([KnownTopology.LDAP, KnownTopology.AD, KnownTopology.Samba])
@pytest.mark.preferred_topology(KnownTopology.LDAP)
def test_sss_override__override_homedir_does_not_take_precedence_over_sss_override(
    client: Client, provider: GenericProvider
):
    """
    :title: override_homedir does not take over sss_override per-user homedir
    :setup:
        1. Create user 'user1'
        2. Start SSSD
        3. Create sss_override for 'user1' setting home to '/home/override_user1'
        4. Restart SSSD
        5. Look up 'user1' to confirm sss_override template is applied
        6. Configure SSSD with 'override_homedir = /home/%u1'
        7. Restart SSSD
    :steps:
        1. Run getent passwd user1 before the override_homedir is applied
        2. Run getent passwd user1 after the override_homedir is applied
    :expectedresults:
        1. Home directory matches '/home/override_user1' - the sss_override override value
        2. Home directory matches '/home/override_user1' - override_homedir did not take a precedence
    :customerscenario: True
    :requirement: IDM-SSSD-TC: ldap_provider: local_overrides: override_homedir does not override sss_override
    """
    provider.user("user1").add(password="Secret123")
    client.sssd.start()
    client.sss_override.user("user1").add(home="/home/override_user1")
    ...
    result = client.tools.getent.passwd("user1")
    assert result is not None, "User 'user1' not found after sss_override!"
    assert result.home == "/home/override_user1", f"sss_override was not applied, got: {result.home}"
    ...
    assert result is not None, "User 'user1' not found after override_homedir!"
    assert (
        result.home == "/home/override_user1"
    ), f"override_homedir took precedence over sss_override, got: {result.home}"
