...
def test_config__reload_config(client: Client):
    """
...
@pytest.mark.config
@pytest.mark.topology(KnownTopologyGroup.AnyProvider)
def test_config__reload_a_section(client: Client):
    """
...
@pytest.mark.importance("critical")
@pytest.mark.topology(KnownTopologyGroup.AnyProvider)
def test_config__reload_added_and_removed_configuration(client: Client):
    """
...
def test_config__genconf_no_such_section(client: Client):
    ...
    This feature has been removed in sssd-2.9.x
    """
    client.sssd.start()
