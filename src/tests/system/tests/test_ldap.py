...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["host1.ldap.test", "host2.ldap.test"],
    ],
)
@pytest.mark.importance("critical")
@pytest.mark.topology(KnownTopology.LDAP)
...
def test_ldap__resolver_provider_lookup_hosts_mixed_ip_versions(client: Client, ldap: LDAP):
    ...
    :steps:
        ...
        2. Lookup hostname and aliases
    :expectedresults:
        1. IP addresses found
        2. Hostname and only ipv6 aliases are found
    :customerscenario: False
    """
    ip_addresses = [
        "2001:db8:1::1",
    ...
    for ip in ipv6_addresses:
        assert ipaddress.IPv6Address(ip), "Not a valid IPv6 address!"
...
def test_ldap__resolver_provider_lookup_networks(client: Client, ldap: LDAP, ip_address, aliases):
    ...
    if result is not None and result.name is not None and result.ip is not None:
        assert "net0" == result.name, f"Network 'net0'  was not found!"
    ...
    if result is not None:
        ...
        for network in aliases:
            assert network in result.aliases, f"Network alias {aliases} for 'net0' was not found!"
