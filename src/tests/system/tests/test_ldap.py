...
def test_ldap__password_change_using_ppolicy(
    ...
    :setup:
        ...
        3. Set "ldap_pwmodify_mode" and "ldap_use_ppolicy"
        4. Start SSSD
    :steps:
...
def test_ldap__password_change_new_passwords_do_not_match_using_ppolicy(
    ...
    """
    :title: Password change when the new passwords do not match using ppolicy
    :setup:
        1. Add user to LDAP
        2. Configure the LDAP ACI to permit user password changes
        3. Set "ldap_pwmodify_mode" and "ldap_use_ppolicy"
...
def test_ldap__password_change_new_password_does_not_meet_complexity_requirements_using_ppolicy(
    ...
    :setup:
        ...
        4. Set "ldap_pwmodify_mode" and "ldap_use_ppolicy"
        5. Start SSSD
    :steps:
...
@pytest.mark.parametrize("use_ppolicy", ["true", "false"])
@pytest.mark.topology(KnownTopology.LDAP)
def test_ldap__password_change_fails_with_invalid_current_password_using_ppolicy(
    client: Client, ldap: LDAP, modify_mode: str, use_ppolicy: str
):
    """
    :title: Password change fails with invalid current password using ppolicy
    :setup:
...
def test_ldap__search_base_is_discovered_and_defaults_to_root_dse(client: Client, ldap: LDAP):
    ...
    for doc in [
        ...
        assert doc in str(log), "Message not found in logs!"
    ...
    log = client.fs.read(client.sssd.logs.domain())
    assert "sdap_set_config_options_with_rootdse" not in log, "Message should not not be in logs!"
