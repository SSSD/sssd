...
def test_cache__invalidate_entries_in_domain_and_timestamps_caches(
    ...
    result = client.ldb.search(path, suffix, filter=f"name={obj}")
    assert result is not None, f"{obj} object not found!"
    for k, v in result.items():
        if isinstance(k, dict):
            expire_time = result.get(str(k)).get("dataExpireTimestamp")
            assert expire_time is not None, f"{dbs} {k} key not found!"
            assert int(expire_time[len(expire_time) - 1]) < 1, f"Expire time for {dbs} does not have a valid value!"
    ...
    for k, v in result.items():
        if isinstance(k, dict):
            expire_time = result.get(str(k)).get("dataExpireTimestamp")
            assert int(expire_time[len(expire_time) - 1]) == 1, f"Expire time for {dbs} has not been cleared!"
