...
def test_cache__invalidate_entries_in_domain_and_timestamps_caches(
    client: Client, provider: GenericProvider, obj: str
):
    """
    ...
    :steps:
        ...
        3. Compare the dateExpireTimestamp value against previous search
    :expectedresults:
        1. Attribute is found
        2. Cache is cleared for the object
        3. Attribute is found and the value has changed
    :customerscenario: False
    """
    # Unable to parametrize provider method, so all object types are created
    # The object names are specific matching the suffix, cannot be renamed without changing the search query.
    ...
    cache = f"/var/lib/sss/db/cache_{client.sssd.default_domain}.ldb"
    timestamps = f"/var/lib/sss/db/timestamps_{client.sssd.default_domain}.ldb"
    suffix = f"cn={client.sssd.default_domain},cn=sysdb"

    # Users and groups objects are cached as the fully qualified name unlike netgroups
    if obj != "netgroup":
        obj = f"{obj}@{client.sssd.default_domain}"

    for dbs in [cache, timestamps]:
        result = client.ldb.search(dbs, suffix, filter=f"name={obj}")
        assert result is not None, f"{obj} object not found!"
        for k, v in result.items():
            if isinstance(k, dict):
                expire_time = result.get(str(k)).get("dataExpireTimestamp")
                client.host.conn.exec(["sss_cache", f"-{obj[0]}", obj])

                assert expire_time != client.ldb.search(dbs, suffix, filter=f"name={obj}").get(str(k)).get(
                    "dataExpireTimestamp"
                ), f"Clearing {obj} cache entry did not update 'dataExpireTimestamp' in '{dbs}'!"
