...
def assert_objects(
    ...
    """
    Check existence of objects, either users, groups or initgroups.

    This is helper function to parameterize the memcache test. The assertions for each
    cache time is different. Looking up 'users, will use 'id', groups will use 'getent group',
...
def assert_group_membership(
    ...
    'initgroups' cache, it will use 'getent initgroups' and check the memberof attribute with group ids. Important,
...
def test_memcache__lookup_objects_by_name(client: Client, provider: GenericProvider, cache: str):
    ...
    :title: Lookup objects, by name and remains in memcache after SSSD is stopped.
      Objects will either be users, group or initgroups.
    :setup:
