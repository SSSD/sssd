...
def test_ipa__hbac_permitted_host_group_members_can_login(client: Client, ipa: IPA):
    ...
    :setup:
      1. Add users
      2. Create host groups and assign hosts to them
      3. Disable default HBAC rule
      4. Create HBAC rules for host groups and restart SSSD
    :steps:
      1. Test login for users in host groups
    :expectedresults:
      1. Users in host groups have access
    :customerscenario: False
    """
    ...
    host_group1 = ipa.hostgroup("client_host_group").add(description="client host group")
    host_group1.add_member(host="client.test")
    host_group2 = ipa.hostgroup("server_host_group").add(description="server host group")
    host_group2.add_member(host="master.ipa.test")

    ipa.hbac("allow_all").disable()

    ipa.hbac("allow_server_group_ssh_access").create(
        description="SSH access for a server host group",
        users="user1",
        hostgroups="server_host_group",
        services="sshd",
    )
    ipa.hbac("allow_client_group_ssh_access").create(
        description="SSH access for a client host group",
        users=["user1", "user2", "user3"],
        hostgroups="client_host_group",
        services="sshd",
    )
    client.sssd.restart()

    assert ipa.auth.ssh.password("user1", "Secret123"), "user1 should succeed!"
    assert client.auth.ssh.password("user1", "Secret123"), "user1 should have access!"
    ...
    assert not ipa.auth.ssh.password("user4", "Secret123"), "user4 should be denied!"
    assert not client.auth.ssh.password("user4", "Secret123"), "user4 should be denied!"
...
def test_ipa__hbac_access_nested_groups(client: Client, ipa: IPA):
    ...
    :setup:
      1. Create users and groups
      2. Add users to respective groups and create nested structure
      3. Disable default HBAC rule
      4. Create HBAC rule for nested groups and restart SSSD
    :steps:
      1. Test login for users in nested groups
      2. Test access for users not in nested groups
    :expectedresults:
      1. Users in nested groups have access
      2. Users not in nested groups are denied access
    :customerscenario: False
    """
    ...
    for user in users:
        ipa.user(user).add()
    group1 = ipa.group("group1").add(description="group1")
    nested_group1 = ipa.group("nested_group1").add(description="nested_group1")
    nested_group2 = ipa.group("nested_group2").add(description="nested_group2")

    group1.add_members([ipa.user("user1"), ipa.user("user2")])
    nested_group1.add_members([ipa.user("user3"), ipa.user("user4")])
    nested_group2.add_members([group1, nested_group1])

    ipa.hbac("allow_all").disable()

    ipa.hbac("nested_group").create(
        description="nested group", groups="nested_group2", hosts="client.test", services="sshd"
    )
    client.sssd.restart()
...
@pytest.mark.importance("high")
@pytest.mark.topology(KnownTopology.IPA)
def test_ipa__hbac_parametrize_user_host_service_cat_all(client: Client, ipa: IPA, hbac: str):
    """
