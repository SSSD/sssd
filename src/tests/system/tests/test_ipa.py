...
def test_ipa__subids_configured(client: Client, ipa: IPA):
    ...
    ), f"Subordinate UID start range {subuid.range_start} does not match: {ipa_sub.uid_start}!"
    assert (
        ipa_sub.uid_size == subuid.range_size
    ), f"Subordinate UID start range {subuid.range_size} does not match: {ipa_sub.uid_size}!"
    assert client.tools.getsubid("user2") is None, "User has subids configured!"

    subgid = client.tools.getsubid("user1", group=True)
    assert (
        ipa_sub.gid_start == subgid.range_start
    ), f"Subordinate GID start range {subgid.range_start} does not match: {ipa_sub.gid_start}!"
    assert (
        ipa_sub.gid_size == subgid.range_size
    ), f"Subordinate GID start range {subgid.range_size} does not match: {ipa_sub.gid_size}!"
    assert client.tools.getsubid("user2", group=True) is None, "User has subids configured!"
