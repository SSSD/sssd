...
def create_ptr_zones(client: Client, provider: GenericProvider) -> None:
    """
    Create PTR zones.
   ...
   :type provider: GenericProvider
    """
    for ip_address in client.net.ip("eth0").addresses:
...
def test_dyndns__enabled(client: Client, provider: GenericProvider):
    ...
    if client.net.nslookup([f"client.{provider.domain}", provider.server]) == 0:
        provider.dns().zone().delete_record(f"client.{provider.domain}")
