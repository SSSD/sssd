...
def test_sssctl__user_fully_qualified_name_show(client: Client, provider: GenericProvider, case_sensitive: bool):
    ...
    if case_sensitive:
        assert " is not present in cache" in result.stdout, f"{lc_user_name} must not be in the cache!"
    else:
...
def test_sssctl__group_fully_qualified_name_show(client: Client, provider: GenericProvider, case_sensitive: bool):
    ...
    cg1 = provider.group("CamelCaseGroup1").add().add_member(camel_user)
    client.sssd.domain["use_fully_qualified_names"] = "True"
    client.sssd.domain["case_sensitive"] = str(case_sensitive)
    client.sssd.start()
    ...
    if case_sensitive:
        assert "is not present in cache" in result.stdout, f"{lc_group_name} must not be in the cache!"
    else:
...
def test_sssctl__debug_level_component(client: Client, provider: GenericProvider, component: str):
    ...
    result = client.host.conn.exec(["sssctl", "debug-level"])
    assert re.search(
        f"^{component}\\s+0x0270", result.stdout, re.MULTILINE
    ), f"Debug level for [{component}] is not set"
    for item in other_components:
        assert not re.search(
            f"^{item}\\s+0x0270", result.stdout, re.MULTILINE
        ), f"Debug level for [{item}] is set to 0x0270 but it should not be"
