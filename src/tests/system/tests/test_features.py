...
def get_sssd_version(client: Client):
    ...
    :rtype: tuple
    """
    rpm = client.host.conn.run('test -f /usr/bin/rpm', raise_on_error=False)
    dpkg = client.host.conn.run('test -f /usr/bin/dpkg-query', raise_on_error=False)
    if rpm.rc == 0:
        ver = client.host.conn.run(r'rpm -q sssd --queryformat "%{VERSION}-%{RELEASE}"').stdout
    elif dpkg.rc !=0:
        ver = client.host.conn.run(r"dpkg-query -f '${Version}' -W sssd").stdout
    else:
...
def test_feature__sss_ssh_knownhosts(client: Client):
    ...
    """
    major, minor, _, _ = get_sssd_version(client)
...
def test_feature__passkey(client: Client):
    ...
    if "Fedora" in client.host.distro_name:
        ...
        expect_passkey = not (client.host.distro_major <= 23 or (client.host.distro_major == 23 and client.host.distro_minor < 10))
    else:
        pytest.skip("Unknown distro, no expectations set for passkey feature presence")

    assert bool(
        client.features["passkey"] == expect_passkey
    ), f"Passkey does not match expectations on {client.host.distro_name} {client.host.distro_major} {client.host.distro_minor}."
