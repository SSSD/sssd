...
def test_access_control_simple__allow_and_deny_nested_group(client: Client, provider: GenericProvider):
    ...
    :setup:
        ...
        2. Create groups
            `allowed_nested_group` and add `group1` as a member; add `user1` as member of `group1`
            `denied_nested_group` and add `group2` as a member; add `user2` as member of `group2`
        3. Configure SSSD with:
            `access_provider = simple`
            `simple_allow_groups = allowed_nested_group`
            `simple_deny_groups = denied_nested_group`
        4. Start SSSD.
    :steps:
        1. Attempt login with `user1`
    ...
    provider.group("allowed_nested_group").add().add_member(g1)

    g2 = provider.group("group2").add().add_member(u2)
    provider.group("denied_nested_group").add().add_member(g2)

    client.sssd.domain["access_provider"] = "simple"
    client.sssd.domain["simple_allow_groups"] = "allowed_nested_group"
    client.sssd.domain["simple_deny_groups"] = "denied_nested_group"
