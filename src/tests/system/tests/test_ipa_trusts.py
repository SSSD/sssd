...
def test_ipa_trusts__lookup_private_group_with_username_override(ipa: IPA, trusted: GenericADProvider):
    ...
    :setup:
        2. Create trusted user "user1"
        3. Clear SSSD cache and logs on IPA server
        4. Add 'login' override for user1@trusted.domain
    :steps:
        1. Clear user cache
        3. Lookup user private group for AD user with override name
    :expectedresults:
        4. Auto private group is resolved for override name
    :customerscenario: True
    """
