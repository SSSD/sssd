...
def test_infopipe__list_by_attr(client: Client, provider: GenericProvider):
    ...
    :setup:
        ...
        5. Add "ldap_user_extra_attrs = extraName:uid" to the domain configuration
        6. Launch SSSD
    :steps:
        1. Call ListByAttr() with "extraName=user-2" and no limit
        2. Call ListByAttr() with "extraName=user-*" and no limit
        3. Call ListByAttr() with "extraName=user-*" and a limit of 2
    ...
    for user in list:
        assert user in model, "ListByAttr('extraName', 'user-*', 2) returned an unexpected element"
