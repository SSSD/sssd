...
def test_infopipe__get_group_properties(client: Client, provider: GenericProvider):
    ...
    :steps:
        ...
        4. Verify initial properties using GetAll() (expect empty users list).
        5. Call UpdateMemberList() to refresh the group's member list.
        6. Get all properties using GetAll() and verify the member list.
        7. Validate core properties (name, GID, users).
    :expectedresults:
        ...
        4. Initial users list is empty.
        5. Member list is updated successfully.
        6. Member list contains object paths for 'user1' and 'user2'.
        7. Properties match: name is 'group-1', GID is 30001, and users list has 2 members.
    :customerscenario: False
    """
    # Setup
    ...
    # Step 4: Verify initial properties (expect empty users list)
    props = group.GetAll("org.freedesktop.sssd.infopipe.Groups.Group")
    assert props["users"] == [], f"Expected empty users list initially, got {props['users']}"
    # Step 5: Update member list
    group.UpdateMemberList()

    # Step 6-7: Get all properties and validate
    props = group.GetAll("org.freedesktop.sssd.infopipe.Groups.Group")
