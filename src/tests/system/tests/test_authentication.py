...
def test_authentication__user_login_with_modified_PAM_provider_is_offline(
    client: Client, provider: GenericProvider, method: str, sssd_service_user: str
):
    """
    ...
    :setup:
        ...
        2. Configure SSSD with "cache_credentials = true" and "krb5_store_password_if_offline = true" and
            "offline_credentials_expiration = 0"
        3. Modify PAM configuration files /etc/pam.d/system-auth and /etc/pam.d/password-auth
        3 Start SSSD
    :steps:
        1. Login as user
        2. Offline, login as user
...
"""
    client.fs.write("/etc/pam.d/system-auth", custom_pam_stack)
    client.fs.write("/etc/pam.d/password-auth", custom_pam_stack)
    client.sssd.start(service_user=sssd_service_user)
