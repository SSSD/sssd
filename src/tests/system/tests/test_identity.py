...
@pytest.mark.topology(KnownTopologyGroup.AnyProvider)
def test_identity__lookup_groupname_with_getent(client: Client, provider: GenericProvider, group_name: str):
    """
    ...
    :customerscenario: False
    """
    if provider.name == "ipa" and group_name != "group1":
        pytest.skip("Skipping test for IPA provider, the characters in the name are not supported!")
    ids = [(group_name, 10001), ("group2", 10002), ("group3", 10003)]
    for group_name, id in ids:
        provider.group(group_name).add(gid=id)
