...
int main(int argc, const char **argv)
{
    struct sss_route_cmd commands[] = {
        ...
        SSS_TOOL_COMMAND("cache-remove", "Backup local data and remove cached content", sssctl_cache_remove),
        SSS_TOOL_COMMAND("cache-expire", "Invalidate cached objects", sssctl_cache_expire),
        SSS_TOOL_COMMAND("cache-index", "Manage cache indexes", sssctl_cache_index),
        SSS_TOOL_DELIMITER("Log files tools:"),
        SSS_TOOL_COMMAND("logs-remove", "Remove existing SSSD log files", sssctl_logs_remove),
        SSS_TOOL_COMMAND("logs-fetch", "Archive SSSD log files in tarball", sssctl_logs_fetch),
