...
static void kcm_op_initialize_cc_add_done(struct tevent_req *subreq);
static void kcm_op_initialize_cc_delete_done(struct tevent_req *subreq);
static void kcm_op_initialize_add_step(struct tevent_req *req);
...
static void kcm_op_initialize_got_byname(struct tevent_req *subreq)
{
    ...
    ret = kcm_ccdb_getbyname_recv(subreq, &state->new_cc);
    talloc_zfree(subreq);
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE,
    ...
    kcm_op_initialize_add_step(req);
}
...
static void kcm_op_initialize_add_step(struct tevent_req *req)
{
    ...
    subreq = kcm_ccdb_add_cc_send(state,
                                  state->ev,
                                  state->op_ctx->kcm_data->db,
                                  state->op_ctx->client,
                                  state->new_cc);
    ...
    tevent_req_set_callback(subreq, kcm_op_initialize_cc_add_done, req);
}
...
static void kcm_op_initialize_cc_add_done(struct tevent_req *subreq)
{
    ...
    ret = kcm_ccdb_add_cc_recv(subreq);
    talloc_zfree(subreq);
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE,
...
static void kcm_op_initialize_got_default(struct tevent_req *subreq)
{
    ...
    struct kcm_op_initialize_state *state = tevent_req_data(req,
                                            struct kcm_op_initialize_state);
    errno_t ret;
    struct kcm_ccache *dfl_cc;

    ret = kcm_ccdb_get_default_recv(subreq, &dfl_cc);
    talloc_zfree(subreq);
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE,
...
static void kcm_op_destroy_getbyname_done(struct tevent_req *subreq)
{
    ...
    ret = kcm_ccdb_getbyname_recv(subreq, &cc);
    talloc_zfree(subreq);
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE,
    ...
    if (cc == NULL) {
        DEBUG(SSSDBG_MINOR_FAILURE, "Nothing to destroy\n");
...
static void kcm_op_store_getbyname_done(struct tevent_req *subreq)
{
    ...
    if (cc == NULL) {
        DEBUG(SSSDBG_MINOR_FAILURE, "No credentials to store\n");
        state->op_ret = ERR_NO_MATCHING_CREDS;
        tevent_req_done(req);
        return;
    }

    ret = kcm_cc_store_creds(state->op_ctx->kcm_data->k5c,
                             cc,
                             state->cred_blob);
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE,
...
static void kcm_op_get_cred_by_uuid_getbyname_done(struct tevent_req *subreq)
{
    ...
    for (crd = kcm_cc_get_cred(cc);
        ...
        if (memcmp(uuid, uuid_in, sizeof(uuid_in)) == 0) {
            break;
        }
        kcm_debug_uuid(uuid);
    }
...
static void kcm_op_get_cache_uuid_list_done(struct tevent_req *subreq)
{
    ...
    ret = kcm_ccdb_list_recv(subreq, state, &cc_list);
    talloc_zfree(subreq);
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE,
    ...
    if (cc_list == NULL || cc_list[0] == NULL) {
    ...
    for (int i = 0; cc_list[i]; i++) {
        uuid = kcm_cc_get_uuid(cc_list[i]);
        if (uuid == NULL) {
            DEBUG(SSSDBG_MINOR_FAILURE, "CC has no UUID, skipping\n");
            continue;
        }

        kcm_debug_uuid(uuid);

        ret = sss_iobuf_write_len(state->op_ctx->reply, uuid, UUID_BYTES);
        if (ret != EOK) {
            DEBUG(SSSDBG_OP_FAILURE, "Error: cannot return UUID\n");
            tevent_req_done(req);
            return;
        }
    }
...
static void kcm_op_get_cache_by_uuid_done(struct tevent_req *subreq)
{
    ...
    ret = kcm_ccdb_getbyuuid_recv(subreq, &cc);
    talloc_zfree(subreq);
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE,
                               ...
                               struct kcm_op_ctx *op_ctx)
{
    ...
    req = tevent_req_create(mem_ctx, &state, struct kcm_op_get_default_ccache_state);
    if (req == NULL) {
        return NULL;
    }
    state->op_ctx = op_ctx;
...
static void kcm_op_get_get_default_done(struct tevent_req *subreq)
{
    ...
    ret = kcm_ccdb_get_default_recv(subreq, &state->cc);
    talloc_zfree(subreq);
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE,
              "Cannot get default ccache [%d]: %s\n",
...
kcm_op_get_default_ccache_reply_step(struct kcm_op_get_default_ccache_state *state)
{
    ...
    if (state->cc != NULL) {
        name = talloc_strdup(state, kcm_cc_get_name(state->cc));
    } else {
        name = talloc_asprintf(state,
                               "%"SPRIuid,
                                cli_creds_get_uid(state->op_ctx->client));
    }
    ...
    ret = sss_iobuf_write_stringz(state->op_ctx->reply, name);
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE,
              "Cannot write output name [%d]: %s\n",
              ret, sss_strerror(ret));
...
static void kcm_op_get_kdc_offset_getbyname_done(struct tevent_req *subreq)
{
    ...
    /* heimdal releases cache here, wtf */
...
static void kcm_op_set_kdc_offset_getbyname_done(struct tevent_req *subreq)
{
    ...
    offset = be32toh(offset_be);

    ret = kcm_cc_set_offset(cc, offset);
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE,
              "Cannot set KDC offset [%d]: %s\n",
              ret, sss_strerror(ret));
        tevent_req_error(req, ret);
        return;
    }
...
static struct kcm_op kcm_optable[] = {
    ...
    { "SET_KDC_OFFSET",      kcm_op_set_kdc_offset_send, NULL },
