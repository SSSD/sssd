                ...
                enum sss_id_type *_type)
{
    ...
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE, "Unable to find ID type [%d]: %s\n",
                        ...
                        struct sized_string **_sz_name)
{
    ...
    domname = result->domain != NULL
            ? result->domain->name : result->well_known_domain;
                ...
                struct sized_string **_sz_name)
{
    ...
    name = ldb_msg_find_attr_as_string(msg, ORIGINALAD_PREFIX SYSDB_NAME, NULL);
    if (name == NULL) {
    ...
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE, "Unable to create sized name [%d]: %s\n",
              ret, sss_strerror(ret));
        return ret;
    }
