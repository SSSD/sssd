...
static errno_t
nss_get_netent(TALLOC_CTX *mem_ctx,
...
static errno_t
nss_get_network_aliases(TALLOC_CTX *mem_ctx,
                        struct sss_domain_info *domain,
                        struct ldb_message *msg,
                        const char *name,
                         ...
                         struct cache_req_result *result)
{
    ...
    for (i = 0; i < result->count; i++) {
        ...
        ret = nss_get_netent(tmp_ctx, result->domain, msg, &name, &addr);
        if (ret != EOK) {
        ...
        ret = nss_get_network_aliases(tmp_ctx, result->domain, msg, name.str,
                                      &aliases, &num_aliases);
        if (ret != EOK) {
            DEBUG(SSSDBG_MINOR_FAILURE,
