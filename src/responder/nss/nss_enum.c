                         ...
                         nss_setent_set_timeout_fn timeout_handler)
{
    ...
    if (state->enum_ctx->ongoing != NULL) {
        ...
        if (ret != EOK) {
            DEBUG(SSSDBG_CRIT_FAILURE, "Unable to register setent "
...
static void nss_setent_internal_done(struct tevent_req *subreq)
{
    ...
    case EOK:
        ...
        if (state->type == CACHE_REQ_NETGROUP_BY_NAME) {
            ...
            ret = sysdb_netgr_to_entries(state->enum_ctx, result[0]->ldb_result,
                       ...
                       struct nss_enum_ctx *enum_ctx)
{
    ...
    if (te == NULL) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Could not set up life timer for "
              "enumeration object.\n");
        return ENOMEM;
    }
                             ...
                             const char *netgroup)
{
    ...
    if (ret != EOK) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Unable to add enumeration context into "
              "table [%d]: %s\n", ret, sss_strerror(ret));
        talloc_free(enum_ctx);
        return NULL;
    }
