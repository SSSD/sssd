                        ...
                        const char **_derb64)
{
    ...
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE, "Unable to convert certificate to pem "
              "[%d]: %s\n", ret, sss_strerror(ret));
        return ret;
    }
                       ...
                       const char **_sid)
{
    ...
    err = sss_idmap_sid_to_bin_sid(nss_ctx->idmap_ctx, sid, &bin_sid, &bin_len);
    if (err != IDMAP_SUCCESS) {
        DEBUG(SSSDBG_OP_FAILURE, "Unable to convert SID to binary [%s].\n", sid);
        return EINVAL;
    }
