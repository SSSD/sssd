                         ...
                         struct cache_req_result *result)
{
    ...
    for (i = 0; i < result->count; i++) {
        ...
        if (ret != EOK) {
            DEBUG(SSSDBG_MINOR_FAILURE, "Unable to get service information, "
        ...
        ret = nss_get_svc_aliases(tmp_ctx, result->domain, msg, name.str,
                                  &aliases, &num_aliases);
        if (ret != EOK) {
            DEBUG(SSSDBG_MINOR_FAILURE, "Unable to get service aliases, "
                  "skipping... [%d]: %s\n", ret, sss_strerror(ret));
            continue;
        }
        ...
        for (j = 0; j < num_aliases; j++) {
            ...
            SAFEALIGN_SET_STRING(&body[rp], aliases[j].str, aliases[j].len, &rp);
        }
