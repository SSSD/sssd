...
errno_t sbus_iterator_write_ifp_extra(DBusMessageIter *iterator,
                                      hash_table_t *table)
{
    DBusMessageIter it_array = DBUS_MESSAGE_ITER_INIT_CLOSED;
    DBusMessageIter it_dict = DBUS_MESSAGE_ITER_INIT_CLOSED;
    ...
    if (ret != EOK) {
        dbus_message_iter_abandon_container_if_open(&it_dict, &it_array);
        dbus_message_iter_abandon_container_if_open(iterator, &it_dict);
    }
