...
/**
 * Convert a base DN string into a domain string. This is, given the
 * basedn represented by the "dc=ad,dc=vm" string, the function should
 * return on *domain the "ad.vm" string.
 * @param memctx The memory context to be used for talloc.
 * @param basedn a not NULL string in the form "dc=a,dc=b,dc=c,..." to be
 * transformed to domain. This is a constant string.
 * @param domain The output string generated by the function with the domain
 * that represent the base DN. It could not be NULL.
 * @return It returns EOK if the operation was successful; EMEMORY when
 * no memory available to finish the operation; EINVAL when input arguments
 * are invalid.
 */
