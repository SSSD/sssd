...
struct _sbus_sss_invoke_in_uuus_out_qus_state {
    struct _sbus_sss_invoker_args_uuus *in;
    struct _sbus_sss_invoker_args_qus out;
    struct {
        enum sbus_handler_type type;
        void *data;
        errno_t (*sync)(TALLOC_CTX *, struct sbus_request *, void *, uint32_t, uint32_t, uint32_t, const char *, uint16_t*, uint32_t*, const char **);
        struct tevent_req * (*send)(TALLOC_CTX *, struct tevent_context *, struct sbus_request *, void *, uint32_t, uint32_t, uint32_t, const char *);
        errno_t (*recv)(TALLOC_CTX *, struct tevent_req *, uint16_t*, uint32_t*, const char **);
    } handler;
...
static void
_sbus_sss_invoke_in_uuus_out_qus_step
    (struct tevent_context *ev,
     struct tevent_timer *te,
     struct timeval tv,
     void *private_data);
...
static void
_sbus_sss_invoke_in_uuus_out_qus_done
   (struct tevent_req *subreq);
...
_sbus_sss_invoke_in_uuus_out_qus_send
   (TALLOC_CTX *mem_ctx,
    ...
    const char **_key)
{
    struct _sbus_sss_invoke_in_uuus_out_qus_state *state;
    struct tevent_req *req;
    const char *key;
    errno_t ret;

    req = tevent_req_create(mem_ctx, &state, struct _sbus_sss_invoke_in_uuus_out_qus_state);
    if (req == NULL) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Unable to create tevent request!\n");
        return NULL;
    }
    ...
    state->in = talloc_zero(state, struct _sbus_sss_invoker_args_uuus);
    if (state->in == NULL) {
        DEBUG(SSSDBG_CRIT_FAILURE,
              "Unable to allocate space for input parameters!\n");
        ret = ENOMEM;
    ...
    ret = _sbus_sss_invoker_read_uuus(state, read_iterator, state->in);
    if (ret != EOK) {
        goto done;
    }

    ret = sbus_invoker_schedule(state, ev, _sbus_sss_invoke_in_uuus_out_qus_step, req);
    if (ret != EOK) {
        goto done;
    }
...
static void _sbus_sss_invoke_in_uuus_out_qus_step
   (struct tevent_context *ev,
    struct tevent_timer *te,
    struct timeval tv,
    void *private_data)
{
    ...
    req = talloc_get_type(private_data, struct tevent_req);
    state = tevent_req_data(req, struct _sbus_sss_invoke_in_uuus_out_qus_state);
    ...
    case SBUS_HANDLER_ASYNC:
        ...
        tevent_req_set_callback(subreq, _sbus_sss_invoke_in_uuus_out_qus_done, req);
        ret = EAGAIN;
        goto done;
    }
...
static void _sbus_sss_invoke_in_uuus_out_qus_done(struct tevent_req *subreq)
{
    struct _sbus_sss_invoke_in_uuus_out_qus_state *state;
    struct tevent_req *req;
    errno_t ret;
