...
class Generator:
    ...
    def GenerateCode(templates, interfaces):
        ...
        class Callers:
            def __init__(self, interfaces, type):
                ...
                self.Setters = InvokerCaller.GatherSetInvokers(interfaces, type)
    ...
    class Base(object):
        ...
        def setOutputArguments(self, tpl, sbus_signature):
            ...
            if tpl.hasLoop("out"):
                for idx, arg in enumerate(sbus_signature.arguments.values()):
                    tpl.add("out", {'type' : DataType.Find(arg.signature).outputCType,
                                    'name' : arg.name,
                                    'index': idx})
        ...
        def getMemberName(self, iface, member):
            ...
            return self.tokenizeValue(self.getInterfaceName(iface), member_name)
        ...
        def hasAny(self, invoker_signature):
            ...
            """
            return bool(invoker_signature.arguments) or self.hasRaw(invoker_signature)

        def hasParsable(self, invoker_signature):
            """
                Return true if the invoker signature has any parsable arguments.
            """
            return not invoker_signature.invokerSignature in ["", "raw"]
        ...
        def toggleKeygen(self, tpl, keys, member, sbus_signature):
            ...
            for idx, arg in args.items():
                ...
                if type.keyFormat is None:
                    raise ValueError(('Data type "%s" does not '
    ...
    class Symbols(Base):
        ...
        def annotation(self, annotation):
            value = "NULL"
            if not annotation.value is None:
            ...
            for signature, args in self.invoker_arguments.items():
                for idx, arg in enumerate(args.values()):
                    ...
                    keys = {"arg-signature": arg.signature,
                            "talloc-context": "mem_ctx, " if type.RequireTalloc else "",
                            "index": idx}
    ...
    class Invokers(Base):
        ...
        def generateSource(self):
            ...
            for invoker in self.invokers.values():
                ...
                tpl.show("if-output-arguments", self.hasParsable(invoker.output))
    ...
    class MethodCalls(Base):
        ...
        def generateInvokers(self):
            ...
            for invoker in self.invokers.values():
                ...
                tpl.show("if-raw-output", self.hasRaw(invoker.output))
                tpl.show("if-empty-input", self.hasEmpty(invoker.input))
                tpl.show("if-has-output", self.hasAny(invoker.output))
                tpl.show("if-use-talloc", self.outputNeedTalloc(invoker.output))
                tpl.show("if-show-dummy", self.showDummy(invoker))
        ...
        def generateCallers(self, tpl):
            for iface in self.interfaces.values():
                for method in iface.methods.values():
                    ...
                    keys = {"token" : self.getMemberName(iface, method),
                            "iface" : iface.name,
                            "method" : method.name,
                            "keygen" : InvokerKeygen.BuildKeygenName(method, method.input),
                            "input-signature": invoker.input.invokerSignature,
                            "output-signature": invoker.output.invokerSignature}
        ...
        def showDummy(self, invoker):
            return self.hasEmpty(invoker.output) and not self.hasParsable(invoker.input)
    ...
    class PropertyCalls(Base):
        ...
        def __init__(self, source, header, interfaces, type, get_invokers, set_invokers):
        ...
        def generate(self):
            self.generateInvokers(self.source.get("get-invoker"), self.get_invokers)
            self.generateInvokers(self.source.get("set-invoker"), self.set_invokers)
        ...
        def generateInvokers(self, tpl, invokers):
            for invoker in invokers.values():
                ...
                if self.hasAny(invoker.input):
                    type = DataType.Find(invoker.input.arguments['value'].signature)
                if self.hasAny(invoker.output):
                    type = DataType.Find(invoker.output.arguments['value'].signature)
        ...
        def generateCallers(self, tpl):
            for iface in self.interfaces.values():
                for property in iface.properties.values():
                    ...
                    keys = {"token" : self.getMemberName(iface, property),
                            "iface" : iface.name,
