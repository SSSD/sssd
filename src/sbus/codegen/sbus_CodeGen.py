...
class CodeGen:
    ...
    class Options:
        def __init__(self,
            ...
            self.IncludeHeaders = IncludeHeaders if IncludeHeaders is not None else []
    ...
    class Templates:
        ...
        class File:
            def __init__(self, name, options):
                ...
                self.template = TemplateFile(options.path('./templates/' + name + '.tpl'))
                self.setHeader()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate sbus server and client code.')
    parser.add_argument('introspection', nargs='+', help="Path to introspection file")
    required = parser.add_argument_group('Required arguments')
    required.add_argument('--sbus', action="store", dest="sbuspath", help="Path to sbus header files as used in #include", required=True)
    required.add_argument('--util', action="store", dest="utilpath", help="Path to util header files as used in #include", required=True)
    required.add_argument('--headers', action="store", dest="headerpath", help="Path to generated header files as used in #include", required=True)
    required.add_argument('--dest', action="store", dest="destpath", help="Path where the generated code will be stored", required=True)
    required.add_argument('--fileprefix', action="store", dest="fileprefix", help="Name prefix for generated files", required=True)
    optional = parser.add_argument_group('Optional generator arguments')
    optional.add_argument('--symbolprefix', action="store", dest="symbolprefix", help="Name prefix for generated global symbols", required=False)
    optional.add_argument('-i', '--include', action='append', dest="include", help="Include header with definition of custom types", required=False)
    cmdline = parser.parse_args()

    opts = CodeGen.Options(SbusHeadersPath = cmdline.sbuspath,
