...
static void sbus_reconnect_attempt_done(struct tevent_req *req);
                       ...
                       void *data)
{
    ...
    req = sbus_connect_init_send(sbus_conn, sbus_conn, sbus_conn->wellknown_name);
    if (req == NULL) {
        ret = ENOMEM;
        goto done;
    }

    tevent_req_set_callback(req, sbus_reconnect_attempt_done, sbus_conn);
    return;
...
static void sbus_reconnect_attempt_done(struct tevent_req *req)
{
    ...
    conn = tevent_req_callback_data(req, struct sbus_connection);

    ret = sbus_connect_init_recv(req);
    talloc_zfree(req);
    /* Issue next attempt or finish. */
    if (ret != EOK) {
        sbus_reconnect(conn);
